...
// Conversation stage map (enum replacement)
...
serve(async (req) => {
  ...
  try {
    ...
    if (isInitialAnalysis) {
      ...
      const lowerStory = (context.userStory ?? '').toLowerCase();
      const isOpenQuestion = lowerStory.includes('what do you suggest')
        || lowerStory.includes('what should')
        || lowerStory.includes('recommend');
...
function extractContextUpdates(
  ...
  if (lowerMessage.includes('kol') ||
      lowerMessage.includes('key opinion leader') ||
  ...
  else if (lowerMessage.includes('hcp') ||
  ...
  if (stage === ConversationStage.OCCASION_DISCOVERY && !detectedIntent.occasion) {
    if (lowerMessage.includes('event') || lowerMessage.includes('congress') || lowerMessage.includes('conference')) {
      ...
      // Extract event name (simple heuristic)
