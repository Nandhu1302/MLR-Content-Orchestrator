...
/**
 * ContentService - converted to plain JS (JSX file) keeping original logic.
 * Note: TypeScript types removed to preserve runtime behavior.
 */
export class ContentService {
  ...
  static async saveContentVariation(assetId, variationData) {
  ...
  static async updateVariation(variationId, updates) {
    const { data, error } = await supabase
      .from('content_variations')
      .update(updates)
      .eq('id', variationId)
  ...
  static mapIntakeAssetTypeToDbType(intakeAssetType) {
    ...
    if (!typeMapping[intakeAssetType]) {
      ...
      if (String(intakeAssetType).toLowerCase().includes('email')) {
        return 'mass-email';
      }
    }
