...
export class StoryIntelligenceMatchingService {
  static async matchIntelligenceToStory(
    context,
    brandId
  ): Promise {
    ...
    try {
      const [
        ...
        VisualsService.fetchMatchingVisuals(brandId, detectedIntent, selectedAssets || []),
      ...
      const dataSourcesUsed = this.calculateDataSources(
        claims, visuals, modules, successPatterns, audienceInsights, channelIntel,
        competitiveIntel, crossChannelJourneys, performancePrediction, marketIntel
      );
      const overallConfidence = this.calculateConfidence(claims, visuals, modules, successPatterns);
  ...
  static fetchAudienceSegmentDetails = AudienceService.fetchAudienceSegmentDetails;
  static fetchMarketingMixRecommendations = ChannelService.fetchMarketingMixRecommendations;
  ...
  static getEmptyIntelligence(): MatchedIntelligence {
    return {
      claims: [],
      visuals: [],
      modules: [],
